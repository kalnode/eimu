...
export default async ({ strapi }) => {
    ...
    const pluginConfigFile = await strapi.config.get('plugin.' + appConfig.app_name)
    const pluginCore = strapi.plugins[appConfig.app_name]
    ...
    try {
        ...
        if (!Object.keys(pluginConfigFile).includes('connection')) {
            ...
            if (!pluginInstanceCached.settings.cronSchedule) {
                ...
                strapi.cron.add({
                    [appConfig.app_name+'_cronIndexing']: {
                        options: {
        ...
        strapi.db.lifecycles.subscribe(async (event) => {
            ...
            if (event.action === 'afterCreate' || event.action === 'afterUpdate') {
                ...
                try {
                    ...
                    const contentTypesForIndexing = await strapi.plugins[appConfig.app_name].services.indexes.getContentTypesForIndexing()
