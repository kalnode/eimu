...
export default ({ strapi }) => ({
    ...
    async ES_createIndex(indexName:estypes.IndexName) {
    ...
    async ES_deleteIndex(indexName:estypes.IndexName) {
        try {
            this.ES_checkConnection()
            const exists = await ES_CLIENT.indices.exists({ index: indexName })
            if (exists) {
    ...
    async ES_cloneIndex(indexName:estypes.IndexName, targetName:estypes.IndexName) {
    ...
    async ES_reindexIndex(indexName:estypes.IndexName, targetName:estypes.IndexName) {
    ...
    async ES_toggleDynamicMapping(indexName:estypes.IndexName) {
    ...
    async ES_getMapping(indexName:estypes.IndexName) {
    ...
    async ES_indexRecordToSpecificIndex_Old({ itemId, itemData }, indexName:estypes.IndexName) {
        try {
            this.ES_checkConnection()
    ...
    async ES_indexRecordToSpecificIndex(index:RegisteredIndex, itemId:string, itemData:any,) {
        try {
            this.ES_checkConnection()
            await ES_CLIENT.index({
