...
exports.default = ({ strapi }) => ({
    ...
    // ADD/UPDATE
    // =================================
    async processStrapiAddUpdateEvent(event) {
        ...
        try {
            ...
            const helper = strapi.plugins[app_config_1.default.app_name].services.helper;
            const strapiTypesForPlugin = await helper.getStrapiTypesForPlugin();
            if (indexes && event.model && event.model.uid && event.model.uid != 'strapi::core-store') {
                indexes.forEach(async (index) => {
                    ...
                    const mappingsForIndex = mappings.filter((x) => x.uuid && index.mappings && index.mappings.includes(x.uuid));
                    const matchedMapping = mappingsForIndex.find((x) => x.content_type === event.model.uid);
                    if (matchedMapping) {
                        // REMOVE
    ...
    // DELETION
    // =================================
    async processStrapiDeleteEvent(event) {
        // TODO: Type event:{} better.
        const pluginInstance = strapi[app_config_1.default.app_name + '_pluginCache'];
        const indexes = pluginInstance.indexes;
        const tasksService = strapi.plugins[app_config_1.default.app_name].services.tasks;
                    ...
                    try {
                        for (let r = 0; r < pendingTasks.length; r++) {
                            ...
                            if (task.record_id) {
                                const pluginInstance = strapi[app_config_1.default.app_name + '_pluginCache'];
                                const indexes = pluginInstance.indexes;
                                const index = indexes.find((x) => x.uuid === task.index_uuid);
                                const mappingForRecordContentType = index.mappings.find((x) => x.content_type === task.content_type);
    ...
    async indexAllRecords(indexUUID) {
        ...
        if (index && index.mappings) {
            for (let i = 0; i < index.mappings.length; i++) {
                const mapping = mappings.find((x) => x.uuid === index.mappings[i]);
                if (mapping) {
...
// INTERNAL FUNCTIONS
// ==================================
const processSingleAddUpdateRecord = async (index, mapping, strapiTypesForPlugin, result) => {
    ...
    for (const [keyType, valueType] of Object.entries(strapiTypesForPlugin)) {
        ...
        for (const [keyField, valueField] of Object.entries(valueType)) {
            // LOOP - Attributes within field
            for (const [keyField2, valueField2] of Object.entries(valueField)) {
                ...
                if (keyField2 === 'whole_raw_object') {
                    let targetType = valueField2['target'];
                    // RELATION EXISTS
...
const addUpdateSingleRecord = async (index, mapping, result) => {
    try {
        ...
        const mappingWithHydratedPresets = await (0, scripts_1.populateMappingPresets)(mappings, mapping);
...
const deleteSingleRecord = async (index, content_type, recordStrapiId) => {
    try {
        const esInterface = strapi.plugins[app_config_1.default.app_name].services.esInterface;
        const logsService = strapi.plugins[app_config_1.default.app_name].services.logs;
        // NOTE: This prepares final ES-side id, e.g. "api::event.event::48"
...
function getIndexItemId({ contentTypeUid, itemId }) {
    return contentTypeUid + '::' + itemId;
}
const extractRecordDataToIndex = (mapping, result) => {
    const finalRecord = {};
...
function modifySubfieldsConfigForExtractor(collectionConfig) {
