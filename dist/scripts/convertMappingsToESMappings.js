"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.convertMappingsToESMappings = void 0;
const _1 = require(".");
const convertMappingsToESMappings = (mappings) => {
    let mappingFieldsFinal = {};
    if (mappings && mappings.length > 0) {
        for (let i = 0; i < mappings.length; i++) {
            let mapping = mappings[i];
            let objectWork = processFields(mapping);
            mappingFieldsFinal = { ...mappingFieldsFinal, ...objectWork };
        }
    }
    return mappingFieldsFinal;
};
exports.convertMappingsToESMappings = convertMappingsToESMappings;
const processFields = (mapping) => {
    let finalObject = {};
    if (mapping.fields) {
        for (const [key, value] of Object.entries(mapping.fields)) {
            if (value.active) {
                if (value.mapping) {
                    finalObject[key] = {
                        type: value.type,
                        index: value.index,
                        properties: processFields(value.mapping)
                    };
                }
                else {
                    finalObject[key] = {
                        type: value.type,
                        index: value.index
                    };
                }
                // TODO: Is this needed?
                (0, _1.removeUndefineds)(finalObject[key]);
            }
        }
    }
    return finalObject;
};
